...
public class IndexerApp {
    ...
    private void sync() throws Exception {
        ...
        DatasetImportServiceCombined importService = new DatasetImportServiceCombined((DatasetImportServiceMyBatis) inj.getInstance(DatasetImportService.class), inj.getInstance(NameUsageIndexService.class), 3);

        try {
            Importer importer = Importer.create(iCfg, datasetKey, registry, importService, nameUsageService, usageService);
            importer.run();
        } finally {
            importService.close();
            hds.close();
