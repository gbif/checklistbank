...
public class DamerauLevenshtein implements StringSimilarity {
  ...
  public static double convertEditDistanceToSimilarity(int edits, String s1, String s2) {
    int length = (s1.length() + s2.length()) / 2;
    double sim = edits > length ? 0 : 100d * (1d - (double) edits / (double) length);
    return sim;
  }

  public double getSimilarity() {
    if (!calculated) setupMatrix();
    return convertEditDistanceToSimilarity( matrix[compOne.length()][compTwo.length()], compOne, compTwo);
  }
